...
import {
  ...
  remainingUpgradeCost, capPE, signingRepBonus
} from '@/lib/career';
...
function DiceFootballApp() {
  ...
  const applyCareerMatchday = (scoreH, scoreA) => {
    ...
    const rep = Math.round((repForMatch(result, ownStrength, rivalStrength) + bonusRep) * 10) / 10;
    ...
    setCareer(c => ({
      ...
      reputation: clampRep(c.reputation + rep),
      seasonLog: [
      ...
      seasonHistory: [
        {
          ...
          clResult: review.clResult, fired: review.fired
        },
        ...(c.seasonHistory || [])
      ]
    }));
